...
'''
def matchesIPv6(addr):
    if( (len(addr.split(":"))==8) or (len(addr.split("::"))==2) ):
        return True
    else:
        return False
...
def encodeIPv6(ipv6_addr_string):
    list_addr=ipv6_addr_string.split('::')
    intaddr=0
    #print(list_addr,len(list_addr))
    if  (len(list_addr)==2):
        back_addr=list_addr[1].split(':')
        front_addr=list_addr[0].split(':')
        j=8-len(front_addr)-len(back_addr)
        add_0=''
        while j > 0:
            add_0 += '0000'
            #print(7-t,intaddr)
            j=j-1
        hexaddr=list_addr[0].replace(':', '')+add_0+list_addr[1].replace(':', '')
    elif len(list_addr)==1:
        hexaddr=ipv6_addr_string.replace(':', '')
    hexaddr=hexaddr[0:32]
    print(hexaddr.decode('hex'))
    return(hexaddr.decode('hex'))
...
def encode(x, bitwidth):
    ...
    #print "x:",x
    #print "len(encoded_bytes):",len(encoded_bytes)
